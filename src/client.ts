...
import { AddressListParams, AddressListResponse, AddressRetrieveResponse, Addresses } from './resources/addresses';
import { Challenge, ChallengeRetrieveResponse } from './resources/challenge';
import { Health, HealthCheckResponse } from './resources/health';
import { Meta, MetaRetrieveResponse } from './resources/meta';
import { RequestKey, RequestKeyCreateParams, RequestKeyCreateResponse } from './resources/request-key';
import { Reverse, ReverseGeocodeParams, ReverseGeocodeResponse } from './resources/reverse';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['ADDRESS_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['ADDRESS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['ADDRESS_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Address API. 
 */
export class Address {
   ...
   * API Client for interfacing with the Address API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['ADDRESS_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['ADDRESS_BASE_URL'] ?? https://address.tom.so] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('ADDRESS_BASE_URL'),
    apiKey = readEnv('ADDRESS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.AddressError(
        'The ADDRESS_API_KEY environment variable is missing or empty; either provide it, or instantiate the Address client with an apiKey option, like new Address({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://address.tom.so`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Address.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('ADDRESS_LOG'), 'process.env[\'ADDRESS_LOG\']', this) ?? defaultLogLevel;
  ...
  protected async authHeaders(opts: FinalRequestOptions, schemes: { apiKeyAuth?: boolean }): Promise<NullableHeaders | undefined> {
  ...
  static AddressError = Errors.AddressError;
   ...
   * Full-text address search and reverse geocoding powered by FTS5 with abbreviation expansion.
   */
  reverse: API.Reverse = new API.Reverse(this);
  /**
...
Address.Challenge = Challenge;
Address.RequestKey = RequestKey;
Address.Addresses = Addresses;
Address.Search = Search;
Address.Reverse = Reverse;
Address.Meta = Meta;
